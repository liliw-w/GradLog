...
### 1. What is gene *SENP7*?{#what_is_gene_SENP7}
...
```{r}
df <- data.frame("disease" = c("crohn's disease (cd)", "Inflammaroty bowel disease (ibd)", "Allergy", "MS"),
                 "#light green regions" = c(51, 48, 43, 20),
                 "#dark green regions" = c(20, 18, 11, 1),
                 stringsAsFactors = FALSE, check.names = FALSE)
df <- cbind(df, "coloc prop" = df[["#dark green regions"]]/df[["#light green regions"]] )
knitr::kable(df)
```

[Note: I added module44 on the above table.]{.ul}
...
![](asset/1213_cd.coloc.Pval.region.summary.png){width="32%"} ![](asset/1213_ibd.coloc.Pval.region.summary.png){width="32%"} ![](asset/1213_Allergy.coloc.Pval.region.summary.png){width="32%"}

[**For MS, there is only one dark green region, module44:7:50308527. This region has 69 SNPs, and corresponds to module 44 and chr 7.**]{.ul}


#### Observations

![](asset/1213_cd_ibd_Allergy_coloc_info.png){width="30%"} ![](asset/1213_cd_ibd_Allergy_coloc_region_overlap.png){width="25%"}
...
```{r add module44}
df <- data.frame("module" = paste0("module", c(25, 51, 153, 156, 44)),
                 "#Genes" = c(123, 61, 15, 13, 70),
                 "Enrichment" = c("some genes are ZNF* genes", "almost all genes are ZNF* genes", "no genes are ZNF* genes", "12 genes are ZNF* genes", "no genes are ZNF* genes"),
                 stringsAsFactors = FALSE, check.names = FALSE)
knitr::kable(df)
```
...
-   Module enrichment, relevant to immune?
    ...
    **Question: These four modules contain many zinc finger genes.**
    ...
    - [**For MS, module 44's contained genes and enrichment result see [here](https://biit.cs.ut.ee/gplink/l/t6CyzBjcS9).**]{.ul}
...
-   SNP variants
    ...
    - [**For MS, cis- genes?**]{.ul}
      ...
      The lead SNP of region module44:7:50308527 is close to gene [*IKZF1*](https://genome.ucsc.edu/cgi-bin/hgTracks?db=hg19&lastVirtModeType=default&lastVirtModeExtraState=&virtModeType=default&virtMode=0&nonVirtPosition=&position=chr7%3A50295000%2D50472798&hgsid=1260587557_latSAw7QoV0gBDVTJRA2kKabpth8).
...
![](asset/1123_1.reg.module25:3:101011537.gene.SENP7.reg.coloc.png){width="45%"} ![](asset/1123_2.reg.module66:12:54685880.gene.NFE2.reg.coloc.png){width="45%"}
...
### 2. Enrichment of colocalized regions {#enrichment-of-colocalized-regions}
...
[Paper](https://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1006646):

```{r fig.cap="enloc equation", out.width="80%"}
knitr::include_graphics(c("asset/1101_enloc_theory.png"), error = FALSE)
```

[Code tutorial](https://github.com/xqwen/fastenloc/tree/master/tutorial):
...
-   DAP can be applied to each LD block independently. [independent LD block](https://academic.oup.com/bioinformatics/article/32/2/283/1743626) provided a working recipe to segment the full genome based on the population-specific LD structures.

-   Example of using enloc for colocalization: [GTEx 2020](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7737656/). Used for "cis-QTL-GWAS colocalization: ENLOC."
...
[Paper-coloc single](https://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1004383), [Paper-coloc multiple](https://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1009440):

```{r fig.cap="coloc equation", out.width="30%"}
knitr::include_graphics(c("asset/1101_coloc_theory.png"), error = FALSE)
```

[Code tutorial](https://chr1swallace.github.io/coloc/):

First, segment the whole genome. Then fine-map each traits by [`susieR`](https://stephenslab.github.io/susieR/articles/index.html), better than `coloc.abf` as it allows multiple causal variants. Finally, coloc.
...
-   Example of using coloc for colocalization:
    ...
    -   [Mu & Li](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-021-02334-x#Sec11)): `coloc` + default prior.
    -   [GTEx 2020](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7737656/): Used for "Colocalization of cis- and trans-QTLs.", `coloc` + default prior. In addition, used for "Colocalization of neutrophil ieQTLs and GWAS traits": `coloc` + `enloc` prior.
...
Check document [here](asset/0708-Correlation%20between%20meta-ed%20gene%20zscores%20from%20eQTLGen.pdf).
...
### Why there are so many eQTLGen signals using PCO\_${\lambda<0.1}$?

Check the QQ-plot of the observed pvalues from eQTLGen using PCO\_${\lambda<0.1}$ (9918 unique SNPs, all eQTLGen variants) and PCO\_${\lambda<1}$ (909 unique SNPs).
...
See the google doc [here](https://docs.google.com/spreadsheets/d/1PizRyY3VnvTHxu3PIM2_2sLbrj0Q_mxzRC9TrIm7vi4/edit?usp=sharing).
...
-   [rs12485738](https://www.ncbi.nlm.nih.gov/snp/rs12485738) : 3:56865776, intron variant of *ARHGEF3*.
-   [rs643381](https://www.ncbi.nlm.nih.gov/snp/rs643381) : 6:139839423. ([rs590856](https://www.ncbi.nlm.nih.gov/snp/rs590856), 6:139844429).
-   [rs149007767](https://www.ncbi.nlm.nih.gov/snp/rs149007767) 7:50370254: , intron variant of *IKZF1*.
-   [rs12718597](https://www.ncbi.nlm.nih.gov/snp/rs12718597) : 7:50428428, intron variant of *IKZF1*.
-   [rs35979828](https://www.ncbi.nlm.nih.gov/snp/rs35979828) : 12:54685880, 500B downstream variant of *NFE2*.
-   [rs7210990](https://www.ncbi.nlm.nih.gov/snp/rs7210990) : 17:16170764, intron variant of *PIGL*.
