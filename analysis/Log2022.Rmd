...
```{r out.width="80%", fig.cap="Figure: Coloc proportion of trans loci of MSig pathways and blood traits, based on 129 originally defined regions and 112 merged regions. Blue: original non-merged regions. Green: merged regions. Dark: coloc.", fig.show='hold',fig.align='center'}
knitr::include_graphics(c("asset/2022/1005_coloc_region_prop_merged.pdf"), error = FALSE)
```


(merged) coloc prop for traits

See numerical result: `/project2/xuanyao/llw/MODULES/MSigDB/coloc_MSigDB/ukbb_all/coloc_region_prop_merged.txt`.


- Number of merged regions that have coloc for each pair of (pathway, trait)

```{r out.width="80%", fig.cap="Figure: Number of coloc merged regions for all pairs of (pathway, trait). Colors: Traits of various types.", fig.show='hold',fig.align='center'}
knitr::include_graphics(c("asset/2022/1005_reg_module_trait_merged.pdf"), error = FALSE)
```


See numerical result for the specific coloc regions of (pathway, trait): `/project2/xuanyao/llw/MODULES/MSigDB/coloc_MSigDB/ukbb_all/coloc_region_summary_all_merged_annot.txt`.

A few important columns:

  1. Region: the coloc region defined by the lead trans-eQTLs in that region and the correpsonding module
  
  2. module, module_annot: the coloc module and its MSig pathway annotation
  
  3. Phenocode, trait: the coloc trait
  
  4. PP.H4.abf: coloc PP4
  
  5. merged_region: the merged region that the region belong to
  
  6. merged_region_n: the number of regions in the merged region


See file `/project2/xuanyao/llw/MODULES/MSigDB/coloc_MSigDB/ukbb_all/coloc_region_summary_M9_HEME.txt` for the coloc regions of only HALLMARK_HEME_METABOLISM.



- Number of (not merged) regions that have coloc for each pair of (pathway, trait)
  
  See [non-merged coloc regions](asset/2022/1005_reg_module_trait.pdf){width="45%"}.
...
```{r out.width="50%", fig.cap="Figure: LEFT: only coloc modules. RIGHT: all modules.", fig.show='hold',fig.align='center'}
knitr::include_graphics(c("asset/2022/0921_h2_enrich_order_by_dist.pdf", "asset/2022/0921_h2_enrich_order_by_dist_all_modules.pdf"), error = FALSE)
```
...
**All results & figures for this section are in folder `/project2/xuanyao/llw/eQTLGen/MSigDB_est_Sigma/`.**


#### ~~Signals before removing any modules based on ratio~~

- ~~Signal file: `/project2/xuanyao/llw/eQTLGen/MSigDB_est_Sigma/postanalysis/signal.txt`~~

- ~~Numbers: 72696 significant (snp, module) pairs, corresponding to 9309 trans-eQTLs and all 50 modules.~~

- ~~Signal distribution~~

  - ~~on modules, [figure](asset/2022/0921_signal_module.pdf)~~
  
  - ~~on chr's, [figure](asset/2022/0921_signal_chr.pdf)~~
  
  - ~~on (module, chr), [figure](asset/2022/0921_signal_module_chr.pdf)~~


- ~~Observation~~

  ~~Inflated signals for the first few large module. Thus, remove these modules based on their $\frac{null \ snps}{module \ size}$ ratios. See updated signals in next section.~~


#### Signals after removing (module, chr) with ratio < 50

In [previous section](#ratio_of_module_and_chr), I gave $\frac{null \ snps}{module \ size}$ ratio for each pair of (module, chr). So here, I remove a (module, chr) from signals if its ratio < 50.
...
- Result files
  ...
  - Signal file: `/project2/xuanyao/llw/eQTLGen/MSigDB_est_Sigma/postanalysis/signal_rm_infl_ratio_50_module_chr.txt`
  
  - Annotation files with module pathway, gwas catalog trait, cis- genes: `/project2/xuanyao/llw/eQTLGen/MSigDB_est_Sigma/postanalysis/signal_cis_genes_w_annot_rm_infl_ratio_50_module_chr.txt`
...
- Signal distribution
  ...
  - on modules, [figure](asset/2022/0921_signal_module_rm_infl_ratio_50_module_chr.pdf)
  
  - on chr's, [figure](asset/2022/0921_signal_chr_rm_infl_ratio_50_module_chr.pdf)
  
  - on (module, chr), [figure](asset/2022/0921_signal_module_chr_rm_infl_ratio_50_module_chr.pdf)

```{r out.width="30%", fig.cap="Figure: LEFT: on modules. MIDDLE: on chr's. RIGHT: on (module, chr).", fig.show='hold',fig.align='center'}
knitr::include_graphics(c("asset/2022/0921_signal_module_rm_infl_ratio_50_module_chr.pdf", "asset/2022/0921_signal_chr_rm_infl_ratio_50_module_chr.pdf", "asset/2022/0921_signal_module_chr_rm_infl_ratio_50_module_chr.pdf"), error = FALSE)
```


- Observation

  Looks like the additionally added module 37 has many signals, I suspect if these signals are inflated. So I also looked at signals after removing modules based on module-based ratios, as in [previous section](#msig_eqtlgen_module_ratio) and what we used in eQTLGen with co-expression modules.
...
- Result files
  ...
  - Signal file: `/project2/xuanyao/llw/eQTLGen/MSigDB_est_Sigma/postanalysis/signal_rm_infl_ratio_50.txt`
  
  - Annotation files with module pathway, gwas catalog trait, cis- genes: `/project2/xuanyao/llw/eQTLGen/MSigDB_est_Sigma/postanalysis/signal_cis_genes_w_annot_rm_infl_ratio_50.txt`
...
- Signal distribution
  ...
  - on modules, [figure](asset/2022/0921_signal_module_rm_infl_ratio_50.pdf)
  
  - on chr's, [figure](asset/2022/0921_signal_chr_rm_infl_ratio_50.pdf)
  
  - on (module, chr), [figure](asset/2022/0921_signal_module_chr_rm_infl_ratio_50.pdf)

```{r out.width="30%", fig.cap="Figure: LEFT: on modules. MIDDLE: on chr's. RIGHT: on (module, chr).", fig.show='hold',fig.align='center'}
knitr::include_graphics(c("asset/2022/0921_signal_module_rm_infl_ratio_50.pdf", "asset/2022/0921_signal_chr_rm_infl_ratio_50.pdf", "asset/2022/0921_signal_module_chr_rm_infl_ratio_50.pdf"), error = FALSE)
```
...
**All results & figures for this section are in folder `/project2/xuanyao/llw/eQTLGen/DGN_est_Sigma/`.**
...
- Result files
  ...
  - Signal file: `/project2/xuanyao/llw/eQTLGen/DGN_est_Sigma/postanalysis/signal_rm_infl_ratio_50.txt`
  
  - Annotation files with module pathway, gwas catalog trait, cis- genes: `/project2/xuanyao/llw/eQTLGen/DGN_est_Sigma/postanalysis/signal_cis_genes_w_annot_rm_infl_ratio_50.txt`
...
```{r out.width="70%", fig.cap="Figure: Signals for (chr, module). Y-axis is chromosome postiion. X-axis is module. Each point is a signal. Point size means -logP.", fig.show='hold',fig.align='center'}
knitr::include_graphics(c("asset/2022/0921_signal_module_chr_pos.pdf"), error = FALSE)
```
...
- #signals for (chr, module)
  ...
  `/project2/xuanyao/llw/eQTLGen/DGN_est_Sigma/plot/signal_module_chr_rm_infl_ratio_50.pdf`

- -logp for (chr postion, module)
...
#### Update ratio plot {#ratio_of_module_and_chr}
