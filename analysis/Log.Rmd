...
file_name = c("LD.prun.in.txt", "LD.prun.in.perm5.txt",
              "LD.prun.in.chr.txt", "LD.prun.in.chr.perm5.txt",
              "LD.prun.in.chr.module.txt", "LD.prun.in.chr.module.perm5.txt")
...
names(uniq_signals) = paste0("QTL.perm", rep(perm.seq, 3))
venn.col = c("pink", "light green", "light blue")
venn.list = list()
for(i in 1:3){
  venn.list[[i]] = venn.diagram(uniq_signals[1:Kperm+(i-1)*Kperm], fill = rep(venn.col[i], Kperm), alpha = rep(0.5, Kperm), lwd =0, print.mode = c('raw', 'percent'), NULL, main = res$FDR[i])
}
